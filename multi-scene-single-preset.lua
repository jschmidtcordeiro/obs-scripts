    ...
    obs.obs_properties_add_button(props, "add_scene", "Add Scene", function(props, property)
        ...
        if scene_name and scene_name ~= "" then
            ...
            for i = 0, count - 1 do
                ...
                if obs.obs_data_get_string(item, "value") == scene_name then
                    scene_exists = true
            ...
            if not scene_exists then
                ...
                obs.obs_data_set_string(new_scene, "value", scene_name)
...
function script_update(new_settings)
    ...
    print("[DEBUG] Current target scenes:")
    for i, scene in ipairs(target_scenes) do
        print(string.format("[DEBUG] %d. %s", i, scene))
    end
    io.flush()
end
